...
#
# 11/16/2017: change function name to calRaster
#+++++++++++++++++++++++++#
...
calRaster <- function(known, isoscape, mask = NULL, interpMethod = 2, NA.value = NA, ignore.NA = T) {
  ...
  nSample <- nrow(known)
  # create two blank vector for storing the isotope of tissue and isoscape
  tissue.iso <- vector("numeric", length = nSample)
  isoscape.iso <- vector("numeric", length = nSample)
  # create a blank vector for storing the location where isoscape does  not have value
  null.iso <- NULL
  ...
  # if there is no isoscape value at known origin location
  if (!is.na(NA.value)){
  ...
  values(sd)[is.na(values(isoscape))] <- NA # replace all no data with NA
