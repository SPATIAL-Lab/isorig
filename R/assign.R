...
assign <- function(rescaled_raster, unknown, mask = NULL, genplot=T) {
  ...
  if (class(unknown) != "data.frame") {
    stop("unknown should be a data.frame, see help page of assign function")
  }
  if (class(unknown) == "data.frame"){
    data <- unknown
  }
  ...
  for (i in 1:n) {
    ...
    filename <- paste("output/assign_Gtif/", indv.id, ".like", ".tif", sep = "")
  ...
  for (i in 1:n) {
    a <- raster(paste("output/assign_Gtif/", data[i, 1], ".like.tif", sep = ""))
    if (genplot ==T){
      par(mfrow=c(1,1))
      plot(a)
    }
  ...
  if (n > 5){
    pdf("./output/output_assign.pdf", width = 20, height = 40)
    par(mfrow = c(ceiling(length(data.class()[, 1])/5), 5))
  } else {
    pdf("./output/output_assign.pdf", width = 20, height = 20)
  }
